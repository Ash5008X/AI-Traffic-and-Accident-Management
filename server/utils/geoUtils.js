/**
 * Geospatial utility functions for calculating distance, bearings, and sector/zone classifications.
 */
...
function classifyZone(centerLat, centerLng, pointLat, pointLng) {
  ...
  return String.fromCharCode(65 + Math.min(zoneIndex, 5)); // 'A' - 'F'
}

/**
 * Filter incidents within radiusKm of center and annotate with zone
 */
function filterAndAnnotate(incidents, centerLat, centerLng, radiusKm = 15) {
  return incidents
    .filter((inc) => {
    ...
    .map((inc) => ({
      ...
      distanceKm: Number(haversineKm(centerLat, centerLng, inc.location.lat, inc.location.lng).toFixed(2)),
    }));
}
